...
public abstract class Explorer {
	...
	public List<ExplorerEntity> openDirectory(ExplorerEntity dir)
		...
		if (dir.isFile())
			throw new CantOpenFileException(dir.getName()
					+ " is not a directory.");
	...
	public void removeEntity(ExplorerEntity file)
			throws CantRemoveFileException {
		/*
		 * if (file.isDirectory()) { for (ExplorerEntity child :
		 * file.listEntities()) removeEntity(child); if (!file.delete()) throw
		 * new CantRemoveFileException("Can't remove direcroty: " +
		 * file.getName()); } if (!file.delete()) throw new
		 * CantRemoveFileException("Can't remove file: " + file.getName());
		 */
		file.delete();
	}
